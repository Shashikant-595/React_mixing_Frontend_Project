...
// const BatchEfficiencyComponent = ({ isDarkMode }) => {
...
const BatchEfficiencyComponent = ({ isDarkMode }) => {
  ...
  // Pie chart options to make legend text white in dark mode
  const chartOptions = {
    plugins: {
      legend: {
        labels: {
  ...
  return (
    <div className={`text-center p-0 rounded-lg  ${isDarkMode ? 'bg-gray-800 text-gray-300' : 'bg-white text-gray-800'}`}>
      <h3 className="text-lg font-semibold underline underline-offset-4 mb-4">
        22 Hour's Mixing Production Efficiency
      </h3>
      {batchCount !== null ? (
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center md:items-start">
          ...
          <div className="md:w-1/2 md:pr-6">
            <div className="grid grid-cols-2 gap-y-3">
              {/* Target Batches */}
              <span className="text-lg font-medium text-start">Target Batches:</span>
              <span className="font-bold">132</span>

              {/* Batches Produced */}
              <span className="text-lg font-medium text-start">Batches Produced:</span>
              <span className="font-bold">{batchCount}</span>

              {/* Efficiency */}
              <span className="text-lg font-medium text-start">Efficiency:</span>
              <span className={`font-bold ${efficiency >= 75 ? 'text-green-400' : efficiency >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
                {efficiency.toFixed(2)}%
              </span>

              {/* Efficiency Lag */}
              <span className="text-lg font-medium text-start">Efficiency Lag By:</span>
              <span className="text-red-400 text-lg font-medium">{(100 - efficiency).toFixed(2)}%</span>
            </div>
          </div>
